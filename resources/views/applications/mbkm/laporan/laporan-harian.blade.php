    ...
    <div class="container">
        <div class="row gx-3">
            @for ($i = 0; $i < 5; $i++)
                ...
                <!-- Modal -->
                <div class="modal fade" id="modalForm_{{ $i }}" tabindex="-1"
                    aria-labelledby="modalLabel_{{ $i }}" aria-hidden="true">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('laporan.harian.store') }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" id="tanggal_{{ $i }}"
                                            name="tanggal" value="{{ $date }}" required readonly>
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3 isi-laporan-container">
                                        <label for="isi_laporan_{{ $i }}" class="form-label">Isi
                                            Laporan</label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.kehadiran').forEach(function(selectElement) {
                selectElement.addEventListener('change', function() {
                    const isiLaporanContainer = this.closest('.modal-body').querySelector(
                        '.isi-laporan-container');
                    if (['libur nasional', 'sakit', 'cuti', 'tidak ada operasional', 'bencana']
                        .includes(this.value)) {
                        isiLaporanContainer.querySelector('label').innerText = 'Keterangan';
                        isiLaporanContainer.querySelector('textarea').setAttribute('placeholder',
                            'Isi keterangan alasan tidak hadir');
                    } else {
                        isiLaporanContainer.querySelector('label').innerText = 'Isi Laporan';
                        isiLaporanContainer.querySelector('textarea').removeAttribute(
                            'placeholder');
                    }
                });
            });
