            ...
            $('#batch-mbkms').DataTable({
                ...
                columns: [
                    ...
                    { data: 'actions', orderable: false, searchable: false, render: function(data, type, row) {
                        const editUrl = `{{ route('batch-mbkms.edit', ':id') }}`.replace(':id', row.id);
                        const deleteUrl = `{{ route('batch-mbkms.destroy', ':id') }}`.replace(':id', row.id);

                        const editButton = row.is_active ? `<a href="${editUrl}" class="btn btn-primary">Edit</a>` : '';
                        const deleteButton = row.is_active ? '' : `
                            <form action="${deleteUrl}" method="POST" style="display: inline;">
                                @csrf
                                @method('delete')
                                <button type="button" class="btn btn-danger" onclick="deleteRow(${row.id})">Delete</button>
